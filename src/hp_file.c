...
int HP_CreateFile(char *fileName){
  ...
  hp_info->blocks_index = 0; //BF_getBlockCounter(file_desc) - 1
...
int HP_CloseFile( HP_info* hp_info ){
  ...
  free(hp_info); //here I free the metadata struct, that came sooner than expected, nice!
  return 0;
}
...
int HP_InsertEntry(HP_info* hp_info, Record record){
  ...
  int block_info_margin = hp_info->max_records*sizeof(Record);
  ...
  if(hp_info->last_block_index == 0){ //if the index of the last block is zero it means that there is only one block and we need to allocate another one
    ...
    CALL_BF(BF_GetBlock(hp_info->file_desc,hp_info->last_block_index,block));
  }
  printf("%d\n", __LINE__);
  ...
  if(block_info->num_o_records < hp_info->max_records){ //if the block can take new records
    ...
    HP_block_info* bi = new_data+  block_info_margin;
  ...
  memcpy(hp_info,info,sizeof(HP_info)); //update the external hp_info structure with the resolution
  printf("%d\n", __LINE__);
...
int HP_GetAllEntries(HP_info* hp_info, int value){
  ...
  for(i = 1; i <= hp_info->last_block_index; i++){
    printf("%d\n", __LINE__);
    ...
    for(j = 0; j < block_info->num_o_records; j++){
      printf("%d\n", __LINE__);
